...
export const Route = createFileRoute("/privacy")({
  ...
  head: () => ({
    meta: [
      { title: "Privacy and Compliance · Remedy Tech Zone" },
      {
        name: "description",
  ...
  return (
    <div className="flex flex-col gap-8">
      <header className="flex flex-col gap-3">
        <p className="text-[0.7rem] font-medium tracking-[0.28em] text-jade uppercase">Legal</p>
        <h1 className="display text-4xl font-medium md:text-5xl">Privacy and Compliance</h1>
        <p className="max-w-xl text-muted-foreground">
          A short, plain notice — no bunker of footnotes. Last updated September 2026.
        </p>
      </header>
      ...
      <div className="flex max-w-2xl flex-col divide-y divide-border">
        {SECTIONS.map((section) => (
          <section key={section.title} className="py-6 first:pt-0">
            <h2 className="display text-2xl font-medium">{section.title}</h2>
